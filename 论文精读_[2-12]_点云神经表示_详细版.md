...
```python
def manhole_detection_pointcloud(lidar_points):
    """
    基于点云的井盖检测
    ...
    # 分割井盖区域
    manhole_mask = varifold_segmentation(varifold, class_label='manhole')
    
    # 提取圆形参数
    circle_params = fit_circle_3d(ground_points[manhole_mask])
...
4. **设计**：设计一个基于Neural Varifold的3D井盖检测系统。
