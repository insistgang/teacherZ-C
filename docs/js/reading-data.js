...
const paperNotesV2 = [
  ...
  },
  {
    ...
    methodHandle: "T-ROF 先解一次 Rudin-Osher-Fatemi (ROF) 恢复问题，再对同一个 ROF 解做多阈值分割。算法反复根据当前分割区域均值 m_i 更新阈值 τ_i = 1/2(m_{i-1}+m_i)，使阈值自动适配相邻类别；projection 只在收敛证明的 modified algorithm 中出现。",
...
const reproDetails = {
  1: {
    ...
    minimalExperiment: "noisy synthetic four-phase image，先解真实凸 ROF 模型（Chambolle-Pock primal-dual），再用 K-means thresholding 得到分割。",
    expectedOutcome: "真实 ROF smoothing 后再 thresholding 明显优于直接 K-means：synthetic four-phase 的 sat_accuracy 达到 0.9961（real-ROF 像素精度），相对 direct K-means 的 0.6590 有大幅提升；保留一个 Gaussian-smoothing 代理 baseline 仅供对比。",
    metrics: ["direct_accuracy","gaussian_baseline_accuracy","sat_accuracy","accuracy_gain"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image"],
    dataRequirement: "synthetic 4-phase 退化图像；不需要下载真实数据。",
    computeRequirement: "CPU，约 1 秒内。",
    implementationRisk: "smoothing 主路径现在是真实 Chambolle-Pock ROF（非 Gaussian 代理），但仍只是 SaT 骨架：无 blur 算子 A、无 H1 项、合成图非论文数据、无论文 baseline，也不覆盖 overview 中其它 SaT 应用分支。",
    ...
    runMetrics: {"direct_accuracy":0.659,"gaussian_baseline_accuracy":0.9799,"sat_accuracy":0.9961,"accuracy_gain":0.3371},
    resultFiles: ["assets/repro/sat_demo.png"],
    fidelityWarning: "Real ROF (Chambolle-Pock) on a synthetic toy four-phase image; no blur operator A, no H1 term, and no paper dataset/baseline. Covers only the SaT skeleton (one of many SaT branches).",
    notes: "SaT main path now solves the real convex ROF model (Chambolle-Pock primal-dual) before K-means thresholding; sat_accuracy is the real-ROF pixel accuracy. A Gaussian-smoothing proxy baseline (gaussian_baseline_accuracy) is kept only for comparison. Synthetic four-phase image; not paper-level.",
  },
  2: {
    reproductionLevel: "partial",
    difficultyScore: 3,
    ...
    minimalExperiment: "synthetic two-phase image，用真实 Chambolle-Pock 求 ROF minimizer u*，按 (m0+m1)/2 阈值化，并与 ground truth 比较。",
    expectedOutcome: "真实 ROF minimizer thresholding 的分割效果清晰：rof_threshold_dice 达到 0.9983（来自真实 Chambolle-Pock 解，非 Gaussian 平滑），优于 noisy direct threshold 的 0.8989；同时记录 PCMS-like energy（perimeter/TV 代理）。",
    metrics: ["direct_dice","gaussian_baseline_dice","rof_threshold_dice","pcms_like_energy"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image"],
    dataRequirement: "synthetic two-phase image。",
    computeRequirement: "CPU，约 1 秒内。",
    implementationRisk: "rof_threshold_dice 现在来自真实凸 ROF 解（Chambolle-Pock），但代码只演示 ROF-thresholding 现象：pcms_like_energy 是无 data-fidelity 项的各向异性 perimeter/TV 代理、非论文数值，无法替代 Theorem 3.4/3.6/3.7 的证明。",
    verificationPlan: "对比 noisy direct threshold 与 ROF-threshold Dice，并记录 perimeter + data fitting 的 PCMS-like energy。",
    resultStatus: "completed",
    experimentId: "sat_rof_trof",
    runtimeSeconds: 0.7709,
    runMetrics: {"direct_dice":0.8989,"gaussian_baseline_dice":0.9962,"rof_threshold_dice":0.9983,"pcms_like_energy":205},
    resultFiles: ["assets/repro/sat_demo.png"],
    fidelityWarning: "Real ROF on a synthetic toy two-phase image; pcms_like_energy is an anisotropic perimeter/TV proxy without the data-fidelity term and is not a paper-reported number. Code cannot substitute for the Theorem 3.4/3.6/3.7 proofs.",
  ...
  4: {
    ...
    minimalExperiment: "blurred+noisy synthetic image，做真实耦合 restoration+segmentation alternating minimization：真实 Gaussian blur 算子 A 的 Fourier 域 Tikhonov g-update (Eq.13)、omega-weighted region means (Eq.14)、Chambolle-Pock multiphase TV region-fitting u-update (Eq.15-16, argmax 硬化)。",
    expectedOutcome: "在恢复图 g 上的联合分割 joint_SA_percent ~91.34% 明显优于直接在退化图上 TV 分割的 direct_SA_percent ~75.90%（SA_gain ~15.44%），restoration_psnr_db ~22.84；恢复 codebook ~[0.20,0.50,0.87] 接近真实 [0.18,0.50,0.84]。",
    metrics: ["direct_SA_percent","joint_SA_percent","SA_gain_percent","restoration_psnr_db","am_outer_iterations"],
    dependencies: ["numpy", "scipy", "matplotlib"],
    dataRequirement: "synthetic 96x96 blurred+noisy image（omega 缺失像素权重已实现，但 headline run 用 blur+noise 设定）。",
    computeRequirement: "CPU，约 1 秒内（AM 外循环 15 步）。",
    implementationRisk: "现在用真实 blur 算子 A 与 Chambolle-Pock TV 多相联合 AM，但仍：单张合成图、无 barcode/cameraman/MRI/colour 数据、无 [43]/[23]/[6] baseline、仅 Gaussian fidelity（未实现 Poisson/impulsive）、未验证收敛定理；SA 不可与论文 99.29/95.66 直接比较。",
    ...
    runtimeSeconds: 1.055,
    runMetrics: {"direct_SA_percent":75.9,"joint_SA_percent":91.34,"SA_gain_percent":15.44,"restoration_psnr_db":22.84,"am_outer_iterations":15},
    resultFiles: ["assets/repro/segmentation_restoration_partial.png"],
    notes: "Real coupled restoration+segmentation alternating minimization: true Gaussian blur operator A with the exact Fourier-domain Tikhonov g-update (Eq.13), omega-weighted region means (Eq.14), and a Chambolle-Pock multiphase TV region-fitting u-update (Eq.15-16, hardened by argmax). The direct baseline runs the same TV multiphase solver on the degraded f (model (6), no restoration). Paper-style Segmentation Accuracy: joint segmentation on the restored g (~91%) clearly beats direct TV segmentation on the blurred+noisy image (~77%); recovered codebook ~[0.20,0.50,0.87] matches the true levels [0.18,0.50,0.84]. Synthetic single image, no paper baselines -> partial, not paper-level.",
    fidelityWarning: "Synthetic 96x96 image only; no barcode/cameraman/MRI/colour data, no [43]/[23]/[6] baselines, Gaussian fidelity only (Poisson/impulsive not implemented), convergence theorems not verified. omega missing-pixel weighting is implemented but the headline run uses the blur+noise regime for reproducibility. SA here is NOT comparable to the paper's reported 99.29/95.66 numbers -- partial, paper-level remains 0/15.",
  },
  5: {
    reproductionLevel: "partial",
    difficultyScore: 4,
    ...
    minimalExperiment: "synthetic 2D vessel network，构造 boundary interval [alpha_i,beta_i]（由 mu/mu_-/mu_+，eq.7-10），只在 candidate boundary set Lambda 上做真实 tight-frame 软阈值：pywt.swt2/iswt2 undecimated stationary wavelet (Haar, level 2, lambda=0.08) 的 A^T T_lambda(A f) 步，配三段阈值+线性对比拉伸 (eq.11-12) 与梯度初始化 (eq.6)。",
    expectedOutcome: "候选边界集 |Lambda| 有限收缩并收敛到二值管状结构：lambda 从 12416 收缩到 0、5 次迭代收敛（converged_empty_lambda=1），即论文关注的 |Lambda^(i)| 收缩序列与迭代数；dice 0.9863 / iou 0.9729 仅为 toy 内部 overlap。",
    metrics: ["dice","iou","raw_dice","raw_iou","lambda_initial","lambda_final","iterations","converged_empty_lambda"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image", "pywavelets"],
    dataRequirement: "synthetic 2D tube network；full reproduction 需要论文使用的真实 2D/3D MRA/CTA tubular structures。",
    computeRequirement: "CPU，约 1 秒内。",
    implementationRisk: "现在用真实 tight frame（pywt undecimated SWT, Haar, 完美重构）做软阈值平滑，但它并非论文精确的 piecewise-linear B-spline framelet / 各向异性 DCWT；数据为合成 2D（无真实 MRA/CTA、无 3D），dice/iou 是论文从未报告的 toy 量。",
    verificationPlan: "记录 Lambda size per iteration、Dice/IoU，并检查候选区域是否随迭代收缩。",
    resultStatus: "completed",
    experimentId: "tubular_tight_frame",
    runtimeSeconds: 0.0848,
    runMetrics: {"dice":0.9863,"iou":0.9729,"raw_dice":0.9823,"raw_iou":0.9653,"lambda_initial":12416,"lambda_final":0,"iterations":5,"converged_empty_lambda":1},
    resultFiles: ["assets/repro/tubular_lambda_shrinkage.png"],
    notes: "Partial reproduction with a REAL tight-frame denoiser: the paper's A^T T_lambda(A f) framelet step is implemented via an undecimated stationary wavelet transform (pywt.swt2/iswt2, Haar, level 2, lambda=0.08) -- itself a tight frame with perfect reconstruction -- applied as soft thresholding only on the candidate boundary set Lambda. The adaptive interval [alpha_i,beta_i] is computed from mu/mu_-/mu_+ (eq.7-10), with three-segment threshold + linear contrast stretch (eq.11-12) and gradient initialisation ||grad f||_1>=eps (eq.6). The paper-relevant quantity is the |Lambda^(i)| shrinkage sequence and iteration count (now driven to |Lambda|=0). dice/iou are TOY internal overlap on synthetic 2D vessels; the paper reports neither Dice nor real 2D/3D MRA/CTA performance.",
    fidelityWarning: "Tight-frame realised as an undecimated stationary wavelet transform (Haar), not the paper's exact piecewise-linear B-spline framelet / anisotropic DCWT; data is synthetic 2D (no real MRA/CTA, no 3D), and dice/iou are toy quantities the paper never reports.",
  },
  6: {
    reproductionLevel: "partial",
    difficultyScore: 4,
    ...
    minimalExperiment: "synthetic 2D vessel network，构造 Lambda boundary set shrinkage：自适应区间 [alpha_i,beta_i] 由 mu/mu_-/mu_+ (eq.7-10)，仅在 Lambda (eq.14) 上做真实 tight-frame 软阈值（pywt.swt2 前向 -> 软阈值 detail bands -> pywt.iswt2 逆变换, Haar, level 2, lambda=0.08），配三段阈值+对比拉伸 (eq.11-13) 与梯度初始化 (eq.6)。",
    expectedOutcome: "Lambda 有限收缩并收敛到二值掩膜（Theorem 1 行为）：lambda 从 12416 收缩到 0、5 次迭代收敛；dice 0.9863 / iou 0.9729 仅为 toy overlap，论文只给 |Lambda^(i)| 收缩与迭代数。",
    metrics: ["dice","iou","raw_dice","raw_iou","lambda_initial","lambda_final","iterations","converged_empty_lambda"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image", "pywavelets"],
    dataRequirement: "toy 用 synthetic 2D vessel network；full reproduction 需要真实 2D/3D MRA 图像。",
    computeRequirement: "toy 为 CPU 秒级；3D MRA 与 tight-frame transform 会显著增加内存和时间。",
    implementationRisk: "tight-frame 平滑现在是真实 pywt SWT (Haar) undecimated frame，但不是论文带方向选择性的 DCWT；仅合成 2D 数据（无 3D MRA/CTA、无 baseline），dice/iou 是论文没有的 toy overlap。",
    ...
    runMetrics: {"dice":0.9863,"iou":0.9729,"raw_dice":0.9823,"raw_iou":0.9653,"lambda_initial":12416,"lambda_final":0,"iterations":5,"converged_empty_lambda":1},
    resultFiles: ["assets/repro/tubular_lambda_shrinkage.png"],
    notes: "Partial reproduction with a REAL tight-frame (undecimated wavelet) denoiser standing in for the paper's tight-frame/DCWT soft-thresholding: pywt.swt2 forward -> soft-threshold detail bands -> pywt.iswt2 inverse (Haar, level 2, lambda=0.08), applied only on the candidate boundary set Lambda (eq.14). Adaptive interval [alpha_i,beta_i] from mu/mu_-/mu_+ (eq.7-10), three-segment threshold + contrast stretch (eq.11-13), gradient init (eq.6). The loop now converges to |Lambda|=0 (finite shrinkage, Theorem 1 behaviour). dice/iou are TOY overlap on a synthetic 2D vessel network and label nothing the paper reports; the paper gives only |Lambda^(i)| shrinkage and iteration counts on real (private) 2D/3D MRA/CTA.",
    fidelityWarning: "Tight-frame denoise uses pywt SWT (Haar) as the undecimated frame, not the paper's DCWT with directional selectivity; synthetic 2D data only (no 3D MRA/CTA, no baselines), and dice/iou are toy overlap absent from the paper.",
  },
  7: {
    reproductionLevel: "partial",
    difficultyScore: 3,
    ...
    minimalExperiment: "degraded 4-quadrant synthetic color image（高度相关 RGB + blur + Gaussian noise + 60% information loss），做 channel smoothing，Stage 2 用真实 sRGB->CIELab (skimage.color.rgb2lab) 把平滑 RGB lifting 成 RGB+Lab 六维特征，再 K-means。",
    expectedOutcome: "真实 CIELab lifting 让 RGB+Lab 6-D 聚类相对 RGB-only 有确定性增益：rgb_lab_accuracy 0.6951 对 rgb_only_accuracy 0.5418（accuracy_gain 0.1533），对应论文 Fig.1/Fig.2 的 motivating case。",
    metrics: ["rgb_only_accuracy","rgb_lab_accuracy","accuracy_gain"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image"],
    dataRequirement: "single synthetic degraded color image；不下载真实数据。",
    computeRequirement: "CPU，约 1 秒内。",
    implementationRisk: "Stage 2 现在用真实 sRGB->CIELab，但 Stage 1 仍是 Gaussian-filter 代理（无 edge-preserving TV、无 deblur 算子 A、无 Poisson 分支），information loss 用 normalized-convolution inpainting 而非论文 TV/H^1 外推，Lab 通道按固定标称 CIELab 范围缩放到 [0,1]；单张合成图、无真实数据集、无 [31]/[39]/[44] baseline、无 Table I/II 量级对齐。",
    verificationPlan: "比较 RGB-only 与 RGB+Lab-like 的 pixel accuracy，并检查输出图中颜色区域边界是否更稳定。",
    resultStatus: "completed",
    experimentId: "slat_color",
    runtimeSeconds: 0.1405,
    runMetrics: {"rgb_only_accuracy":0.5418,"rgb_lab_accuracy":0.6951,"accuracy_gain":0.1533},
    resultFiles: ["assets/repro/slat_rgb_vs_rgblab.png"],
    notes: "Partial SLaT on a degraded synthetic 4-quadrant color image with highly correlated RGB channels (blur + Gaussian noise + 60% information loss). Stage 2 uses real sRGB->CIELab (skimage.color.rgb2lab), giving the RGB+Lab 6-D lifting a clear, deterministic accuracy gain over RGB-only K-means (the paper's Fig. 1 / Fig. 2 motivating case). Stage 1 still uses a Gaussian smoothing proxy instead of the convex Mumford-Shah/TV primal-dual solver, so this is partial, not paper-level.",
    fidelityWarning: "Stage 1 is a Gaussian-filter proxy (no edge-preserving TV, no deblur operator A, no Poisson branch); information loss is filled by normalized-convolution inpainting rather than the paper's TV/H^1 Stage-1 extrapolation. Lab channels are rescaled to [0,1] by fixed nominal CIELab ranges (not per-image min-max). Single synthetic image, no real-image dataset, no external [31]/[39]/[44] baselines, no Table I/II magnitude alignment. Not paper-level.",
  },
  8: {
    ...
    minimalExperiment: "synthetic lat-lon (equirectangular) sphere-like image，跑真实 wavelet tight-frame WSSA-style pipeline：pywt undecimated SWT 软阈值 (A^T T_lambda A, eq 3.1/3.10) 做去噪 + 离散 spherical gradient (eq 2.5) + 真实 boundary-interval shrinkage (eq 3.3-3.8)。",
    expectedOutcome: "真实 SWT 去噪 RMS-to-clean 增益 denoise_gain_db 3.3291，boundary 区间 5 步收敛 (wssa_iterations=5) 且 |Lambda| 从 8226 收缩到 0（论文可验证的少迭代收敛）；dice 0.9529，kmeans_dice 1.0 仅为透明对照——SNR=30dB 下该强度 toy 可分，无法展示论文的方向性优势。",
    metrics: ["dice","kmeans_dice","denoise_gain_db","lambda_initial","lambda_final","lambda_shrink_ratio","interval_halving_ratio","wssa_iterations","snr_db","epsilon"],
    dependencies: ["numpy", "scipy", "matplotlib", "pywavelets"],
    dataRequirement: "toy lat-lon image；paper-level 需要 Earth map、light probe、solar data、spherical retina 等。",
    computeRequirement: "toy 为 CPU 秒级；paper-level spherical wavelet transforms 依赖专门库和更多调参。",
    implementationRisk: "现在用真实 pywt undecimated SWT 平面 tight frame 替代球面谐波小波，但 lat-lon 等角网格非真正 S^2 等角采样，未用 S2LET/SSHT/SO3 axisymmetric/directional/curvelet/hybrid 栈；dice 是论文不报告的 toy 内部指标，K-means 是简化强度聚类而非真实球面数据上的 MATLAB kmeans。",
    verificationPlan: "记录 Dice 和 spherical-gradient threshold，图中标注这是 approximation toy。",
    resultStatus: "completed",
    experimentId: "sphere_wavelet_toy",
    runtimeSeconds: 0.0953,
    runMetrics: {"dice":0.9529,"kmeans_dice":1,"denoise_gain_db":3.3291,"lambda_initial":8226,"lambda_final":0,"lambda_shrink_ratio":0,"interval_halving_ratio":0.8132,"wssa_iterations":5,"snr_db":30,"epsilon":0.02},
    resultFiles: ["assets/repro/sphere_wavelet_toy.png"],
    notes: "Real wavelet tight-frame WSSA-style pipeline on a lat-lon grid: pywt undecimated SWT soft-threshold (A^T T_lambda A, eq 3.1/3.10) replaces the Gaussian denoise proxy (RMS-to-clean gain 3.3291 dB); discrete spherical gradient (eq 2.5) replaces the max(cos,0.2) clip; real boundary-interval shrinkage iteration (eq 3.3-3.8) converges in 5 steps with |Lambda| 8226->0, the paper's verifiable few-iteration convergence. K-means dice reported only for transparency (1.0); at SNR=30 dB this intensity toy is separable so K-means stays competitive and the paper's directional advantage cannot be shown here. Planar SWT approximates the sphere sampling; true spherical-harmonic wavelets (S2LET/SSHT/SO3) still absent.",
    fidelityWarning: "Planar pywt SWT (undecimated tight frame) stands in for spherical-harmonic wavelets; equiangular lat-lon grid is not true S^2 equiangular sampling and no S2LET/SSHT/SO3 axisymmetric/directional/curvelet/hybrid stack is used. Dice is a toy-internal metric the paper does not report; K-means here is a simplified intensity clustering, not MATLAB kmeans on real spherical data.",
  },
  9: {
    reproductionLevel: "partial",
    difficultyScore: 4,
    ...
    minimalExperiment: "paper-like Three-Moon (R^2) 数据，SVM warm init + RBF k-NN graph，求真实 graph total-variation (l1) 凸模型（Chambolle-Pock primal-dual）+ Algorithm-1 beta-doubling 外循环，argmax projection；对照 raw K-means 与 graph-Laplacian (l2)。",
    expectedOutcome: "真实 graph-TV (l1) 同时优于 l2-Laplacian 与初始化：tv_accuracy 0.9957，相对 laplacian_accuracy 0.922 增益 0.0738、相对 initial_accuracy 0.895 增益 0.1007，raw K-means 仅 0.817。",
    metrics: ["kmeans_accuracy","initial_accuracy","laplacian_accuracy","tv_accuracy","tv_gain_over_laplacian","tv_gain_over_init","outer_iterations"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-learn"],
    dataRequirement: "paper-like Three-Moon (R^2) 数据；不下载 benchmark。",
    computeRequirement: "CPU，约 1 秒内。",
    implementationRisk: "现在是真实 graph-TV (l1) Chambolle-Pock primal-dual（非 Laplacian 近似），但仍非 paper-level：Three Moon 保留在 R^2 而非 R^100、仅此一个数据集（无 COIL/Opt-Digits/MNIST）、缩减到 N=750、无 CVM/GL/MBO/TVRF 对照、无 10-run 平均。",
    verificationPlan: "比较 warm init 与 smoothing 后 accuracy，并保存 before/after decision colors。",
    resultStatus: "completed",
    experimentId: "graph_classification",
    runtimeSeconds: 0.3239,
    runMetrics: {"kmeans_accuracy":0.817,"initial_accuracy":0.895,"laplacian_accuracy":0.922,"tv_accuracy":0.9957,"tv_gain_over_laplacian":0.0738,"tv_gain_over_init":0.1007,"outer_iterations":2},
    resultFiles: ["assets/repro/graph_classification_before_after.png"],
    notes: "Real graph total-variation (l1) convex model solved by Chambolle-Pock primal-dual with SVM warm init and Algorithm-1 beta-doubling outer loop, on a paper-like Three-Moon (R^2) dataset; compared against raw K-means and the graph-Laplacian (l2) model. graph-TV outperforms both.",
    fidelityWarning: "Real graph-TV (l1) Chambolle-Pock primal-dual on a paper-like Three-Moon (R^2, RBF k-NN graph, SVM warm init) -- NOT paper-level: Three Moon kept in R^2 not R^100, only this one dataset (no COIL/Opt-Digits/MNIST), reduced N=750, no CVM/GL/MBO/TVRF comparison baselines, no 10-run averaging. paper-level remains 0/15.",
  },
  10: {
    reproductionLevel: "partial",
    difficultyScore: 4,
    ...
    minimalExperiment: "paper-like Three-Moon (R^2)，跑期刊版真实 graph-TV (l1) primal-dual smoothing + argmax projection：按 Eq.15/Algorithm 1-2 的 decoupled per-class subproblems 与 beta-doubling refinement（SVM warm init + RBF k-NN graph）。",
    expectedOutcome: "真实 graph-TV (l1) 击败 l2-Laplacian 与 raw baseline：tv_accuracy 0.9957，相对 laplacian 0.922 增益 0.0738、相对 init 0.895 增益 0.1007。",
    metrics: ["kmeans_accuracy","initial_accuracy","laplacian_accuracy","tv_accuracy","tv_gain_over_laplacian","tv_gain_over_init","outer_iterations"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-learn"],
    dataRequirement: "paper-like Three-Moon (R^2)；paper-level 需要 high-dimensional data and point clouds benchmarks。",
    computeRequirement: "toy 为 CPU 秒级；benchmark graph construction 和 primal-dual iteration 更重。",
    implementationRisk: "现在已实现真实 graph-TV (l1) Chambolle-Pock primal-dual 的 decoupled per-class 子问题与 beta-doubling，但仍单一合成数据集（Three Moon 在 R^2 非 R^100、N=750）、无 COIL/Opt-Digits/MNIST 等 benchmark-scale 数据、无 CVM/GL/MBO/TVRF 论文 baseline、无 10-run 平均。",
  ...
  11: {
    ...
    minimalExperiment: "~10% 低频偏置欠采样 Fourier 逆问题（Shepp-Logan phantom），求真实 l1-wavelet MAP（FISTA forward-backward, db8 analysis prior），并用论文 HPD threshold gamma'_alpha=f+g+sqrt(16 log(3/alpha))sqrt(N)+N + superpixel 二分得到 local credible interval length map。",
    expectedOutcome: "真实 db8 MAP 明显优于 dirty image：map_psnr 22.2878、map_snr 9.1586，相对 dirty_snr 7.4821 的 snr_gain_over_dirty_db 1.6766；HPD threshold gamma_alpha_hpd 6180.66、mean_interval_length 0.3709。",
    metrics: ["map_psnr","map_snr","dirty_snr","snr_gain_over_dirty_db","sampling_rate","noise_sigma","map_runtime_seconds","lci_runtime_seconds","gamma_alpha_hpd","mean_interval_length"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image", "pywavelets"],
    dataRequirement: "标准 Shepp-Logan phantom + Fourier mask；不下载 M31/BrainWeb 数据。",
    computeRequirement: "CPU，约 1 秒内。",
    implementationRisk: "现在用真实 l1-wavelet (db8) MAP via FISTA + 真实 HPD-threshold 二分 local credible intervals，但仍：标准 phantom 非论文 M31/BrainWeb/RI 数据、无真实 RI (NUFFT) 算子、无 auto-mu、无 SARA dictionary、无 Px-MALA/MYULA 采样；O(10^5) 加速比与论文指标不可比。",
    verificationPlan: "记录 MAP PSNR/SNR、gamma alpha toy 值和 interval length map。",
    resultStatus: "completed",
    experimentId: "map_uq_toy",
    runtimeSeconds: 0.4757,
    runMetrics: {"map_psnr":22.2878,"map_snr":9.1586,"dirty_snr":7.4821,"snr_gain_over_dirty_db":1.6766,"sampling_rate":0.0979,"noise_sigma":0.019635,"map_runtime_seconds":0.0606,"lci_runtime_seconds":0.3229,"gamma_alpha_hpd":6180.6593,"mean_interval_length":0.3709},
    resultFiles: ["assets/repro/map_uq_reconstruction_uncertainty.png"],
    notes: "Real l1-wavelet MAP (FISTA forward-backward, db8 analysis prior) on an undersampled Fourier inverse problem, with the paper's HPD threshold gamma'_alpha=f+g+sqrt(16 log(3/alpha))sqrt(N)+N and local credible intervals by superpixel bisection. MAP beats the dirty-image baseline by the reported SNR gain. Uses a synthetic toy test image (Shepp-Logan phantom), not the paper's M31/BrainWeb data; no auto-mu, no SARA dictionary, no Px-MALA.",
    fidelityWarning: "Real l1-wavelet (Daubechies-8) MAP via FISTA + real local credible intervals via HPD-threshold bisection, on a standard Shepp-Logan phantom under ~10% low-frequency-biased Fourier undersampling (SNR=30dB). NOT the paper's M31/BrainWeb/RI data; no true radio-interferometric (NUFFT) operator, no auto-mu estimation, and no Px-MALA/MYULA sampler. Metrics and the O(10^5) speedup claim are not comparable to the paper.",
  },
  12: {
    reproductionLevel: "toy",
    difficultyScore: 5,
    ...
    minimalExperiment: "~10% 欠采样 Fourier 逆问题（Shepp-Logan phantom），求真实 l1-wavelet (db8) MAP via forward-backward splitting，并算真实 HPD credible region threshold + local credible intervals；这实现了优化/UQ 机制，但不含论文的 proximal-MCMC 采样。",
    expectedOutcome: "MAP + HPD-UQ 机制可跑通：map_snr 9.1586、相对 dirty_snr 7.4821 的 snr_gain 1.6766，HPD threshold 与 mean_interval_length 0.3709 的 interval map 可生成；runtime 与论文大规模结果不可比。",
    metrics: ["map_psnr","map_snr","dirty_snr","snr_gain_over_dirty_db","sampling_rate","noise_sigma","map_runtime_seconds","lci_runtime_seconds","gamma_alpha_hpd","mean_interval_length"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image", "pywavelets"],
    dataRequirement: "Shepp-Logan phantom + Fourier mask；full reproduction 需要 RI visibility data 和 measurement operator。",
    computeRequirement: "CPU 秒级；full proximal-MCMC 需要大量样本和诊断，普通笔记本不适合完整跑。",
    implementationRisk: "现在用真实 l1-wavelet (db8) MAP via FISTA + 真实 HPD-threshold 二分 local credible intervals，但 NOT 论文的 proximal-MCMC 采样器（MYULA/Px-MALA），无真实 RI measurement operator，无 M31/Cygnus A/W28/3C288 数据；指标与论文大规模结果不可比。",
    ...
    runMetrics: {"map_psnr":22.2878,"map_snr":9.1586,"dirty_snr":7.4821,"snr_gain_over_dirty_db":1.6766,"sampling_rate":0.0979,"noise_sigma":0.019635,"map_runtime_seconds":0.0606,"lci_runtime_seconds":0.3229,"gamma_alpha_hpd":6180.6593,"mean_interval_length":0.3709},
    resultFiles: ["assets/repro/map_uq_reconstruction_uncertainty.png"],
    notes: "Real l1-wavelet (db8) MAP via forward-backward splitting on a synthetic toy ~10% undersampled Fourier problem, plus real HPD credible region threshold and local credible intervals. This implements the optimisation/UQ machinery but NOT the paper's proximal-MCMC samplers (MYULA/Px-MALA), nor a true RI measurement operator, nor M31/Cygnus A/W28/3C288 data. Runtime is not comparable to the paper's large-scale results.",
    fidelityWarning: "Real l1-wavelet (Daubechies-8) MAP via FISTA + real local credible intervals via HPD-threshold bisection, on a standard Shepp-Logan phantom under ~10% low-frequency-biased Fourier undersampling (SNR=30dB). NOT the paper's M31/BrainWeb/RI data; no true radio-interferometric (NUFFT) operator, no auto-mu estimation, and no Px-MALA/MYULA sampler. Metrics and the O(10^5) speedup claim are not comparable to the paper.",
  },
  13: {
    reproductionLevel: "toy",
    ...
    minimalExperiment: "与 RI UQ I 相同的 ~10% 欠采样 Fourier 逆问题（Shepp-Logan phantom），用真实 db8 l1-wavelet MAP（forward-backward, Algorithm 1 style）+ 真实 concentration-inequality HPD threshold (Eq.19) + 二分 local credible intervals (Eq.26-28)。",
    expectedOutcome: "真实 MAP-based UQ 快速给出重建+不确定性：map_runtime_seconds 0.0606、lci_runtime_seconds 0.3229，map_snr 9.1586；论文的 O(10^5) MAP-vs-MCMC 加速比未在此复现。",
    metrics: ["map_psnr","map_snr","dirty_snr","snr_gain_over_dirty_db","sampling_rate","noise_sigma","map_runtime_seconds","lci_runtime_seconds","gamma_alpha_hpd","mean_interval_length"],
    dependencies: ["numpy", "scipy", "matplotlib", "scikit-image", "pywavelets"],
    dataRequirement: "Shepp-Logan phantom + Fourier mask；paper-level 需要 M31/Cygnus A/W28/3C288 等 RI images。",
    computeRequirement: "CPU 秒级；paper-level 需高效 convex optimization。",
    implementationRisk: "现在用真实 db8 l1-wavelet MAP + 真实 concentration-inequality HPD threshold + 二分 local credible intervals，但无真实 RI 算子/NUFFT、无 Px-MALA baseline、无 hypothesis testing、非论文数据；O(10^5) MAP-vs-MCMC 加速比未复现。",
    ...
    runMetrics: {"map_psnr":22.2878,"map_snr":9.1586,"dirty_snr":7.4821,"snr_gain_over_dirty_db":1.6766,"sampling_rate":0.0979,"noise_sigma":0.019635,"map_runtime_seconds":0.0606,"lci_runtime_seconds":0.3229,"gamma_alpha_hpd":6180.6593,"mean_interval_length":0.3709},
    resultFiles: ["assets/repro/map_uq_reconstruction_uncertainty.png"],
    notes: "Real MAP-based UQ: db8 l1-wavelet MAP (forward-backward, Algorithm 1 style), real concentration-inequality HPD threshold (Eq.19) and local credible intervals by bisection (Eq.26-28), on a synthetic toy test image. No true RI operator/NUFFT, no Px-MALA baseline, no hypothesis testing, and not the paper's data; the O(10^5) MAP-vs-MCMC speedup is not reproduced here.",
    fidelityWarning: "Real l1-wavelet (Daubechies-8) MAP via FISTA + real local credible intervals via HPD-threshold bisection, on a standard Shepp-Logan phantom under ~10% low-frequency-biased Fourier undersampling (SNR=30dB). NOT the paper's M31/BrainWeb/RI data; no true radio-interferometric (NUFFT) operator, no auto-mu estimation, and no Px-MALA/MYULA sampler. Metrics and the O(10^5) speedup claim are not comparable to the paper.",
  },
  14: {
    reproductionLevel: "toy",
    ...
    minimalExperiment: "可见度切成 B=8 blocks，跑真实 online forward-backward RI 重建（论文 Algorithm 2, analysis form）：算子 Phi_k=M_k F 为变密度欠采样正交 2-D Fourier，稀疏基 Psi 为正交 db8 wavelet frame，prox 为 wavelet 软阈值；block b 到达时贡献累积 partial gradient 然后 discard，仅保留 running (mask,dirty) 摘要与当前估计，对照 offline 全数据 FB baseline。",
    expectedOutcome: "online 与 offline 达到近乎一致 SNR（online_snr_db 24.558 vs offline_snr_db 24.6404, rel_diff ~3.3e-3），均远超 dirty_snr_db 15.1086；online 峰值可见度存储 storage_ratio_eta_s 0.1253 ~ 1/B（peak_stored 154 / total_M 1229）——论文核心的 storage/quality 取舍。",
    metrics: ["offline_snr_db","online_snr_db","dirty_snr_db","online_offline_rel_diff","num_blocks_B","total_measurements_M","peak_stored_measurements_online","storage_ratio_eta_s"],
    dependencies: ["numpy", "matplotlib", "scikit-image", "pywavelets"],
    dataRequirement: "synthetic sky + 变密度欠采样 Fourier mask blocks；paper-level 需要真实 RI visibility blocks。",
    computeRequirement: "toy 为 CPU 秒级；SKA big-data setting 需要流式算子和内存监控。",
    implementationRisk: "solver 现在是真实 online forward-backward（db8 wavelet prox + per-block 累积 partial gradient + discard），但 RI 测量算子用合成 sky 上的 on-grid 欠采样正交 FFT 近似，data-term 1/(2 sigma^2) 折进 mu；缺真实 M31/Cygnus A/W28/3C288 sky、Puy 变密度半 Fourier 平面 10% 采样、NUFFT/w-projection (PURIFY) 算子、论文规模 (B 50..500, i_max=50) 与 Table 1 numerics。",
    verificationPlan: "比较 offline/online PSNR/SNR 与 peak stored measurements，并输出 storage-quality 图。",
    resultStatus: "completed",
    experimentId: "online_ri_toy",
    runtimeSeconds: 0.1104,
    runMetrics: {"offline_snr_db":24.6404,"online_snr_db":24.558,"dirty_snr_db":15.1086,"online_offline_rel_diff":0.003347,"num_blocks_B":8,"total_measurements_M":1229,"peak_stored_measurements_online":154,"storage_ratio_eta_s":0.1253},
    resultFiles: ["assets/repro/online_ri_storage_quality.png"],
    notes: "Real online forward-backward RI reconstruction (paper Algorithm 2, analysis form). Measurement operator Phi_k = M_k F is an undersampled orthonormal 2-D Fourier transform (variable-density mask near the uv-origin a la Puy et al., ~30% coverage, 30 dB input noise); sparsity basis Psi is an orthonormal Daubechies-8 wavelet frame (pywt) so the prox of mu*||Psi^H x||_1 is wavelet soft-thresholding (eqs. 38/40). Visibilities are split into B=8 blocks; block b arrives at FB step b and contributes the accumulated partial gradient sum_{k<=b} Phi_k^H(Phi_k x - y_k) (precomputed per-block dirty maps Phi_k^H y_k, Remark 4.2), then is discarded -- only the running (mask,dirty) summary and current estimate are kept. After all blocks arrive the online run keeps iterating to i_max=50 (optional extra iterations, Algorithm 1/2). A standard offline FB baseline uses all M visibilities every step. Result: online and offline reach near-identical SNR (rel.diff ~3e-3) and both clearly beat the un-regularised dirty image, while online's peak visibility storage is eta_s = max_k M_k / M ~ 1/B -- the paper's central storage/quality trade-off. reproductionLevel stays 'toy': a synthetic sky and on-grid masked-FFT approximate the real RI operator; true non-coplanar/w-projection NUFFT visibility operators and real telescope observations remain the gap.",
    fidelityWarning: "toy: the solver is now the real online forward-backward algorithm (L1 Daubechies-8 wavelet prox + per-block accumulated partial gradient + discard), but the RI measurement operator is approximated by an on-grid undersampled orthonormal FFT on a synthetic sky, and the data-term 1/(2 sigma^2) is folded into mu. Missing for paper-level: real M31/Cygnus A/W28/3C288 sky + Puy variable-density half-Fourier-plane 10% sampling, NUFFT/w-projection (PURIFY) operator, paper scales (B in {50..500}, i_max=50, mu=1e4 in MATLAB units), and the Table 1 / M31 SNR~14.2946 dB numerics.",
  },
  15: {
    reproductionLevel: "toy",
    difficultyScore: 5,
    ...
    minimalExperiment: "d=10 各向同性高斯似然 + uniform-box prior（有闭式 evidence），跑 proximal nested sampling：约束采样用真实 MYULA / proximal-Langevin step（约束集示性函数的 Moreau-Yosida envelope，梯度 = (x - proj_Btau(x))/lambda，即论文 Eq.46 的 L2-ball denoising prox）+ Metropolis-Hastings 校正强制 L(x)>L*。",
    expectedOutcome: "真实 MYULA 约束采样在 rejection 会失效的维度上仍准确：absolute_log_error 0.025（estimated -13.8614 vs reference -13.8365），远优于 rejection baseline 的 1.2109（error_reduction 1.1859）；live_points 100, iterations 1200, myula_steps_per_draw 60。",
    metrics: ["dimension","estimated_log_evidence","reference_log_evidence","absolute_log_error","rejection_baseline_log_error","error_reduction_vs_rejection","live_points","iterations","myula_steps_per_draw"],
    dependencies: ["numpy", "scipy", "matplotlib"],
    dataRequirement: "d=10 Gaussian likelihood under uniform-box prior；paper-level 需要 high-dimensional imaging model comparison。",
    computeRequirement: "CPU 秒级；paper-level nested sampling 需要大量 constrained samples。",
    implementationRisk: "现在用真实 MYULA proximal-Langevin 约束采样器 + MH 校正（非纯 rejection），但仅 d=10 且约束是解析 L2-ball (denoising) constraint：无稀疏 l1/wavelet 先验、无 Fourier sensing、无高维 (10^3-10^6) imaging model selection、无 entropy error bars。",
    ...
    runtimeSeconds: 3.8424,
    runMetrics: {"dimension":10,"estimated_log_evidence":-13.8614,"reference_log_evidence":-13.8365,"absolute_log_error":0.025,"rejection_baseline_log_error":1.2109,"error_reduction_vs_rejection":1.1859,"live_points":100,"iterations":1200,"myula_steps_per_draw":60},
    ...
    notes: "Proximal nested sampling on a d=10 isotropic-Gaussian likelihood under a uniform-box prior with closed-form evidence. Constrained sampling uses a real MYULA / proximal-Langevin step: the Moreau-Yosida envelope of the constraint set's characteristic function (gradient = (x - proj_Btau(x))/lambda, an L2-ball projection, the paper's Eq.46 denoising prox) plus a Metropolis-Hastings correction that enforces L(x)>L* exactly -- not pure rejection. Beats the old rejection baseline (err 0.025 vs 1.211) at a dimension where rejection sampling fails (~250 stale replacements). Still a low-dimensional toy analytic check (synthetic Gaussian model), not the paper's high-dimensional imaging model-selection benchmark.",
    fidelityWarning: "Real MYULA proximal-Langevin constrained sampler with MH correction, but only d=10 with an analytic L2-ball (denoising) constraint; no sparsity-promoting l1/wavelet prior, no Fourier sensing, no high-dimensional (10^3-10^6) imaging model selection, no entropy error bars. Not paper-level.",
  }
};
