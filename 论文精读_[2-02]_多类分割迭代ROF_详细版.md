...
```python
def manhole_multiclass_detection(image):
    """
    多类井盖检测（迭代ROF）
    """
    # 数据项计算
    ...
    f_other = 1 - f_manhole - f_road - f_sidewalk
    
    f_data = np.stack([f_manhole, f_road, f_sidewalk, f_other], axis=-1)
