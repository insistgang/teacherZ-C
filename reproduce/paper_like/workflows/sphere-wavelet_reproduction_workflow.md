...
- **当前 runMetrics**（确定性，派生自 `common.SEED`）：`dice = 0.9529`，`kmeans_dice = 1.0`，`denoise_gain_db = 3.3291`，`lambda_initial = 8226`，`lambda_final = 0`，`lambda_shrink_ratio = 0.0`，`interval_halving_ratio ≈ 0.8132`，`wssa_iterations = 5`，`snr_db = 30.0`，`epsilon = 0.02`，`status = completed`；`runtime_seconds` 为亚秒级 wall-clock（随运行波动，不作复现指标，不 pin 具体小数）。
