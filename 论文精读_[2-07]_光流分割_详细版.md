...
```python
def video_manhole_detection(frame1, frame2):
    """
    基于运动分割的井盖检测
    """
    # 1. 运动分割
    ...
    # 3. 在静态区域内进行圆形检测
    manhole_candidates = detect_circles_in_mask(frame1, static_region)
    
    # 4. 利用光流验证
    for candidate in manhole_candidates:
        flow_consistency = check_flow_consistency(u, v, candidate)
        if flow_consistency > threshold:
            confirm_manhole(candidate)
    
    return confirmed_manholes
```
