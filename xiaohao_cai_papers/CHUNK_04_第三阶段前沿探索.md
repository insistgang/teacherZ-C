...
| 方法类别 | 具体技术 | 应用场景 | 关联论文 |
|:---|:---|:---|:---|
| 参数高效微调 | LoRA/Adapter/Prefix Tuning | 大模型微调 | [3-01] |
| 张量分解 | tCURLoRA (张量CUR分解) | 高维参数微调 | [3-02] |
| 自监督学习 | 图神经网络自监督 | 图表示学习 | [3-03] |
| 张量近似 | Sketching Tucker分解 | 大规模张量分解 | [3-04] |
| 双边Sketching | Two-Sided Sketching | 张量加速计算 | [3-05] |
| 多模态学习 | 语言-雷达对比学习 | 跨模态检索 | [3-06] |
| 多专家系统 | GAMED知识适应解耦 | 虚假新闻检测 | [3-07] |
| 运动生成 | Mogo残差量化Transformer | 3D人体运动生成 | [3-08] |
| 迁移学习 | TransNet动作识别 | 视频动作分类 | [3-09] |
| 架构融合 | CNN-ViT混合动作识别 | 视频理解 | [3-10] |
| 概念级XAI | TCAV概念激活向量 | 可解释性评估 | [3-11] |
| 多层次解释 | 多模态XAI框架 | 复杂模型解释 | [3-12] |
| 专家解耦 | GAMED解耦策略 | 多任务学习 | [3-13] |

### 技术演进脉络

```
张量分解体系:                    可解释AI体系:
[3-04]Tucker分解                 [1-06]XAI综述
     ↓                              ↓
[3-05]双边Sketching         [3-12]多模态XAI
     ↓                              ↓
[3-02]tCURLoRA (ICML)       [3-11]概念级XAI (TPAMI)
(参数高效微调突破)              (标准化评估框架)

多模态学习体系:
[3-07]GAMED多专家
     ↓
[3-06]Talk2Radar (Oral) ←──开创性语言-雷达多模态
     ↓
[3-12]多模态XAI

动作识别演进:
[3-09]TransNet迁移学习 ←──相关──→ [3-10]CNN-ViT融合
```

### 关键技术提取

#### 1. tCURLoRA张量分解微调
- **问题**: 传统LoRA矩阵分解难以建模高维参数结构
- **方法**: 张量CUR分解 W = W₀ + C × U × R，保留实际行列
- **创新**: 更好建模高维结构、可解释性强、适合增量更新
- **代码参考**: `tCURLoRA算法模板`

#### 2. Talk2Radar语言-雷达多模态
- **问题**: 如何实现自然语言与4D毫米波雷达的交互
- **方法**: CLIP/ViT语言编码 + PointNet+雷达编码 + 对比学习对齐
- **创新**: 首个语言-雷达多模态数据集，端到端跨模态框架
- **代码参考**: `contrastive_learning_loss()`模板

#### 3. TCAV概念级可解释性
- **问题**: XAI评估缺乏标准化，像素级解释不够语义化
- **方法**: 概念激活向量(CAV) + 概念贡献量化
- **创新**: 建立概念级解释的标准化评估框架
- **代码参考**: `compute_tcav()`算法模板

#### 4. Sketching Tucker分解
- **问题**: 传统HOOI算法复杂度高 O(dⁿ)
- **方法**: 双边Sketching策略 O(ndr)
- **创新**: 行+列方向压缩，核心张量快速计算
- **代码参考**: 张量分解库(TensorLy等)

#### 5. GAMED多专家解耦
- **问题**: 多模态虚假新闻检测的专家协同
- **方法**: 视觉专家 + 语言专家 + 跨模态专家 + 知识适应门控
- **创新**: 动态权重分配，专家解耦学习
- **代码参考**: 多专家网络架构
...
| 论文ID | 可复现性 | 难度 | 建议 |
|:---:|:---:|:---:|:---|
| [3-01] | ⭐⭐⭐⭐⭐ | 低 | PEFT库(HuggingFace)直接可用 |
| [3-02] | ⭐⭐⭐⭐ | 中 | tCURLoRA需在张量维度扩展LoRA |
| [3-03] | ⭐⭐⭐⭐⭐ | 中 | PyG DGL等图神经网络框架成熟 |
| [3-04] | ⭐⭐⭐⭐ | 高 | Sketching Tucker需自定义实现 |
| [3-05] | ⭐⭐⭐⭐ | 高 | 双边Sketching算法需专门优化 |
| [3-06] | ⭐⭐⭐ | 高 | 需自建语言-雷达配对数据集 |
| [3-07] | ⭐⭐⭐⭐ | 中 | 多专家架构设计清晰 |
| [3-08] | ⭐⭐⭐ | 高 | 残差量化层次Transformer复杂 |
| [3-09] | ⭐⭐⭐⭐⭐ | 中 | 迁移学习策略成熟 |
| [3-10] | ⭐⭐⭐⭐⭐ | 中 | CNN+ViT混合架构标准实现 |
| [3-11] | ⭐⭐⭐⭐ | 中 | TCAV有开源实现(captum) |
| [3-12] | ⭐⭐⭐⭐ | 中 | 多层次解释需设计层级结构 |

### 方法间关联图

```
[3-04]Tucker分解 ←──基础──→ [3-05]双边Sketching
     │                              │
     └──────────┬───────────────────┘
                ↓
           [3-02]tCURLoRA ←──应用──→ [3-01]LoRA综述
                │
                └──────────→ 医学图像分割(与[2-20]等关联)

[3-07]GAMED多专家
     ↓
[3-06]Talk2Radar ←──开创性──→ 语言-雷达多模态
     │                              │
     ↓                              ↓
[3-12]多模态XAI            [3-11]概念级XAI
     │                              │
     └──────────┬───────────────────┘
                ↓
           可解释AI标准化

[3-09]TransNet ←──相关──→ [3-10]CNN-ViT
     │                              │
     └──────────┬───────────────────┘
                ↓
           [1-07]动作识别综述(跨Chunk关联)
```

### 推荐学习路径

1. **张量基础**: 先学习[3-04]Tucker分解和[3-05]Sketching的理论基础
2. **微调前沿**: 掌握[3-02]tCURLoRA的张量分解微调方法
3. **多模态突破**: 研究[3-06]Talk2Radar的语言-雷达跨模态创新
4. **可解释性**: 深入[3-11]TCAV概念级XAI的标准化评估框架
