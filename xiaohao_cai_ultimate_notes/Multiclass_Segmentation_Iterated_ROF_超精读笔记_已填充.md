...
T-ROF 先解 Rudin-Osher-Fatemi (ROF) 恢复问题，再对 ROF 解做多阈值分割。算法反复根据当前分割区域均值 m_i 更新阈值 τ_i = 1/2(m_{i-1}+m_i)，用投影后的阈值序列继续迭代，使阈值自动适配相邻类别。

## 关键模型或公式

threshold update: τ_i = 1/2(m_{i-1} + m_i); ROF/T-ROF energy uses Total Variation plus linear or quadratic data term.
...
论文给出 projected T-ROF algorithm 在特定假设下阈值序列收敛的定理；K=2 时模型与 Chan-Vese 之间有等价/联系，并带有调整后的正则参数解释。
